...
def main(data: dict, config: dict, logger, model, datafile):
    ...
    config_annotated["assessment-mode"]="verify"
    ...
    for index, task_data in enumerate(data):
        ...
        try:
            ...
            if not isinstance(result, dict) or len(result) != 12:
                ...
                result_default = result["default"]
                result_default_no_fsl = result["default_no_fsl"]

                result_simple_verify = result["simple_verify"]
                result_complex_verify = result["complex_verify"]
                result_default_verify = result["default_verify"]
